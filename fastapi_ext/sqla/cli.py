...
def generate(app_name: str, autogenerate: bool = False):
    ...
    # print(head)
    if head:
        command.revision(cfg, head=f"{app_info.name}@head", autogenerate=autogenerate)
    else:
        command.revision(
            cfg,
            head=f"base",
            branch_label=app_info.name,
            version_path=f"{app_info.dir}/versions",
